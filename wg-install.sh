...
select_ipv4_subnet() {
	if [ "$auto" = 0 ]; then
		echo
		echo "Enter the first three octets of the IPv4 subnet for clients (e.g., 10.7.0, 192.168.10, 10.6.9):"
		read -rp "Subnet [10.7.0]: " input_subnet
		if [ -z "$input_subnet" ]; then
			ipv4_subnet="10.7.0"
		else
			until check_pvt_ip "$input_subnet.1" && [[ "$input_subnet" =~ ^([0-9]{1,3}\.){2}[0-9]{1,3}$ ]]; do
				echo "Invalid subnet. Must be a valid private IPv4 range (e.g., 10.x.x, 172.16-31.x, 192.168.x)."
				read -rp "Subnet [10.7.0]: " input_subnet
				[ -z "$input_subnet" ] && input_subnet="10.7.0"
			done
			ipv4_subnet="$input_subnet"
		fi
	else
		ipv4_subnet="10.7.0"
	fi
...
select_ipv6_mode() {
	if [[ -n "$ip6" ]]; then
		if [ "$auto" = 0 ]; then
			echo
			echo "IPv6 detected. How should IPv6 be handled for clients?"
			echo "   1) Shared / NAT (Default - assigns fddd:2c4... and masquerades)"
			echo "   2) Unique / Routed (Assigns from a globally routable /64 prefix you provide)"
			read -rp "Mode [1]: " ipv6_mode
			if [ "$ipv6_mode" = "2" ]; then
				echo "Enter your routed IPv6 /64 prefix (e.g., 2001:db8:1234:abcd):"
				read -rp "Prefix: " custom_ipv6_prefix
				custom_ipv6_prefix=$(echo "$custom_ipv6_prefix" | sed 's/:*$//')
				ipv6_prefix="$custom_ipv6_prefix"

				echo "What do you want for the interface address suffix? (e.g., 1:: or feed:face:dead:beef)"
				read -rp "Suffix [1::]: " custom_ipv6_suffix
				if [ -z "$custom_ipv6_suffix" ]; then
					ipv6_interface_suffix=":1::"
				else
					[[ "$custom_ipv6_suffix" != :* ]] && custom_ipv6_suffix=":${custom_ipv6_suffix}"
					ipv6_interface_suffix="$custom_ipv6_suffix"
				fi
			else
...
[Interface]
Address = ${ipv4_subnet}.1/24$([[ -n "$ipv6_prefix" ]] && echo ", ${ipv6_prefix}${ipv6_interface_suffix}/64")
...
create_firewall_rules() {
	if systemctl is-active --quiet firewalld.service; then
		...
		firewall-cmd -q --zone=trusted --add-source=${ipv4_subnet}.0/24
		firewall-cmd -q --permanent --add-port="$port"/udp
		firewall-cmd -q --permanent --zone=trusted --add-source=${ipv4_subnet}.0/24
		# Set NAT for the VPN subnet
		firewall-cmd -q --direct --add-rule ipv4 nat POSTROUTING 0 -s ${ipv4_subnet}.0/24 ! -d ${ipv4_subnet}.0/24 -j MASQUERADE
		firewall-cmd -q --permanent --direct --add-rule ipv4 nat POSTROUTING 0 -s ${ipv4_subnet}.0/24 ! -d ${ipv4_subnet}.0/24 -j MASQUERADE
		if [[ -n "$ipv6_prefix" ]]; then
			firewall-cmd -q --zone=trusted --add-source=${ipv6_prefix}::/64
			firewall-cmd -q --permanent --zone=trusted --add-source=${ipv6_prefix}::/64
			if is_ipv6_nat; then
				firewall-cmd -q --direct --add-rule ipv6 nat POSTROUTING 0 -s ${ipv6_prefix}::/64 ! -d ${ipv6_prefix}::/64 -j MASQUERADE
				firewall-cmd -q --permanent --direct --add-rule ipv6 nat POSTROUTING 0 -s ${ipv6_prefix}::/64 ! -d ${ipv6_prefix}::/64 -j MASQUERADE
			fi
		fi
	else
...
[Service]
Type=oneshot
ExecStart=$iptables_path -w 5 -t nat -A POSTROUTING -s ${ipv4_subnet}.0/24 ! -d ${ipv4_subnet}.0/24 -j MASQUERADE
ExecStart=$iptables_path -w 5 -I INPUT -p udp --dport $port -j ACCEPT
ExecStart=$iptables_path -w 5 -I FORWARD -s ${ipv4_subnet}.0/24 -j ACCEPT
ExecStart=$iptables_path -w 5 -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$iptables_path -w 5 -t nat -D POSTROUTING -s ${ipv4_subnet}.0/24 ! -d ${ipv4_subnet}.0/24 -j MASQUERADE
ExecStop=$iptables_path -w 5 -D INPUT -p udp --dport $port -j ACCEPT
ExecStop=$iptables_path -w 5 -D FORWARD -s ${ipv4_subnet}.0/24 -j ACCEPT
...
remove_firewall_rules() {
	...
	if systemctl is-active --quiet firewalld.service; then
		ip=$(firewall-cmd --direct --get-rules ipv4 nat POSTROUTING | grep "\-s ${ipv4_subnet}.0/24 '"'"'!'"'"' -d ${ipv4_subnet}.0/24" | grep -oE '[^ ]+$')
		# Using both permanent and not permanent rules to avoid a firewalld reload.
		firewall-cmd -q --remove-port="$port"/udp
		firewall-cmd -q --zone=trusted --remove-source=${ipv4_subnet}.0/24
		firewall-cmd -q --permanent --remove-port="$port"/udp
		firewall-cmd -q --permanent --zone=trusted --remove-source=${ipv4_subnet}.0/24
		firewall-cmd -q --direct --remove-rule ipv4 nat POSTROUTING 0 -s ${ipv4_subnet}.0/24 ! -d ${ipv4_subnet}.0/24 -j MASQUERADE
		firewall-cmd -q --permanent --direct --remove-rule ipv4 nat POSTROUTING 0 -s ${ipv4_subnet}.0/24 ! -d ${ipv4_subnet}.0/24 -j MASQUERADE
		if grep -qsE '([a-fA-F0-9]{1,4}:)+.*\/64' "$WG_CONF"; then
			ip6=$(firewall-cmd --direct --get-rules ipv6 nat POSTROUTING | grep "\-s ${ipv6_prefix}::/64 '"'"'!'"'"' -d ${ipv6_prefix}::/64" | grep -oE '[^ ]+$')
			firewall-cmd -q --zone=trusted --remove-source=${ipv6_prefix}::/64
			firewall-cmd -q --permanent --zone=trusted --remove-source=${ipv6_prefix}::/64
			if is_ipv6_nat; then
				firewall-cmd -q --direct --remove-rule ipv6 nat POSTROUTING 0 -s ${ipv6_prefix}::/64 ! -d ${ipv6_prefix}::/64 -j MASQUERADE
				firewall-cmd -q --permanent --direct --remove-rule ipv6 nat POSTROUTING 0 -s ${ipv6_prefix}::/64 ! -d ${ipv6_prefix}::/64 -j MASQUERADE
			fi
		fi
	else
...
new_client() {
	select_client_ip
	specify_ip=n
	if [ "$auto" = 0 ] && [ "$add_client" = 0 ]; then
		echo
		read -rp "Do you want to specify an internal IPv4 address for the new client? [y/N]: " specify_ip
		until [[ "$specify_ip" =~ ^[yYnN]*$ ]]; do
			echo "$specify_ip: invalid selection."
			read -rp "Do you want to specify an internal IPv4 address for the new client? [y/N]: " specify_ip
		done
		if [[ ! "$specify_ip" =~ ^[yY]$ ]]; then
			echo "Using auto assigned IPv4 address ${ipv4_subnet}.$octet."
		fi
	fi
	if [[ "$specify_ip" =~ ^[yY]$ ]]; then
		echo
		read -rp "Enter the last octet for the new client's IPv4 address (${ipv4_subnet}.[2-254]): " input_octet
		until [[ "$input_octet" =~ ^([2-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-4])$ ]] \
			&& ! grep AllowedIPs "$WG_CONF" | cut -d "." -f 4 | cut -d "/" -f 1 | grep -q "^$input_octet$"; do
			if [[ ! "$input_octet" =~ ^([2-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-4])$ ]]; then
				echo "Invalid octet. Must be an integer between 2 and 254."
			else
				echo "The IP address ${ipv4_subnet}.$input_octet is already in use. Please choose another one."
			fi
			read -rp "Enter the last octet (${ipv4_subnet}.[2-254]): " input_octet
		done
		octet="$input_octet"
	fi
	...
	if [[ -n "$ipv6_prefix" ]]; then
		if is_ipv6_nat; then
			client_ipv6_suffix="::$octet"
		else
			specify_ipv6=n
			if [ "$auto" = 0 ] && [ "$add_client" = 0 ]; then
				echo
				read -rp "Do you want to specify a custom IPv6 address suffix for this client? [y/N]: " specify_ipv6
			fi
			if [[ "$specify_ipv6" =~ ^[yY]$ ]]; then
				echo
				read -rp "Enter IPv6 suffix for the new client (e.g. feed:beef:2:face or 1::2): " input_v6_suffix
				[[ "$input_v6_suffix" != :* ]] && input_v6_suffix=":${input_v6_suffix}"
				client_ipv6_suffix="$input_v6_suffix"
				while grep -q "${ipv6_prefix}${client_ipv6_suffix}/128" "$WG_CONF"; do
					echo "That IPv6 address is already in use. Please choose another."
					read -rp "Enter IPv6 suffix: " input_v6_suffix
					[[ "$input_v6_suffix" != :* ]] && input_v6_suffix=":${input_v6_suffix}"
					client_ipv6_suffix="$input_v6_suffix"
				done
			else
				# Randomly generate 4-block hex suffix simulating SLAAC/DHCPv6 allocation
				client_ipv6_suffix=$(printf ":%x:%x:%x:%x" $((RANDOM%65536)) $((RANDOM%65536)) $((RANDOM%65536)) $((RANDOM%65536)))
				while grep -q "${ipv6_prefix}${client_ipv6_suffix}/128" "$WG_CONF"; do
					client_ipv6_suffix=$(printf ":%x:%x:%x:%x" $((RANDOM%65536)) $((RANDOM%65536)) $((RANDOM%65536)) $((RANDOM%65536)))
				done
				if [[ ! "$specify_ipv6" =~ ^[yY]$ ]]; then
					echo "Using auto-generated IPv6 suffix: $client_ipv6_suffix"
				fi
			fi
		fi
	fi
...
[Interface]
Address = ${ipv4_subnet}.$octet/24$([[ -n "$ipv6_prefix" ]] && echo ", ${ipv6_prefix}${client_ipv6_suffix}/64")
DNS = $dns
PrivateKey = $key
...
if [[ -e "$WG_CONF" ]]; then
	ipv4_subnet=$(grep -m 1 '^Address' "$WG_CONF" | grep -oE '([0-9]{1,3}\.){3}' | sed 's/\.$//')
	if [[ -z "$ipv4_subnet" ]]; then ipv4_subnet="10.7.0"; fi
	
	ipv6_prefix=$(grep -m 1 '^# IPV6_PREFIX' "$WG_CONF" | cut -d " " -f 3)
	if [[ -z "$ipv6_prefix" ]]; then
		ipv6_full=$(awk -F, '/^Address/{print $2}' "$WG_CONF" | cut -d/ -f1 | sed 's/ //g')
		if [[ -n "$ipv6_full" ]]; then
			if [[ "$ipv6_full" =~ ^[fF][cdCD] ]]; then
				ipv6_prefix="fddd:2c4:2c4:2c4"
			else
				ipv6_prefix=$(echo "$ipv6_full" | grep -oE '^([0-9a-fA-F]{1,4}:){3}[0-9a-fA-F]{1,4}')
			fi
		else
			ipv6_prefix=""
		fi
